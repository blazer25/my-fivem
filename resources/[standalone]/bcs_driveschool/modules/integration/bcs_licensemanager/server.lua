...
CreateThread(function()
    local new = lib.checkDependency('bcs_licensemanager', '3.25.1')

    local function GetLicenses()
        if new then
            return exports.bcs_licensemanager:GetLicensesList()
        end
        local p = promise:new()
...
RegisterNetEvent('bcs_licensemanager:integration:server:GiveTheory', function(typeTheory)
    local source = source
    Server.GiveTheory(source, typeTheory)
end)
