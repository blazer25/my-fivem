...
utils.DeleteVehicleEntity = function(vehicle)
    Client.config.removeKey(vehicle)
    SetEntityAsMissionEntity(vehicle, true, true)
...
---@param show boolean
---@param message? string
function utils.HelpText(show, message)
    Client.config.helpText(show, message)
end
...
function utils.CreatePoint(data, cb)
    return lib.points.new({
        coords = data.coords,
        distance = 3,
        onEnter = function()
            utils.HelpText(true, Locale.open_school .. ' ' .. data.name)
        end,
        onExit = function()
            utils.HelpText(false)
        end,
        ...
        nearby = function(self)
            self.marker:draw()
            if self.currentDistance < 2.0 and IsControlJustPressed(0, 38) and cb then
                cb()
            end
        end
    })
...
function utils.RemovePoint(point)
    if point then
        point:remove()
    end
end
