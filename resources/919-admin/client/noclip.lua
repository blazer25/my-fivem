...
if Config.NoClipType == 3 then
    ...
    local DisabledControls = function()
        ...
        if ESCEnable then EnableControlAction(0, 200, true) end
    end

    local IsControlAlwaysPressed = function(inputGroup, control) return IsControlPressed(inputGroup, control) or IsDisabledControlPressed(inputGroup, control) end

    local IsPedDrivingVehicle = function(ped, veh) return ped == GetPedInVehicleSeat(veh, -1) end
    ...
    local CheckInputRotation = function()
        ...
        if (rotation.x + yValue > MinY) and (rotation.x + yValue < MaxY) then newX = rotation.x + yValue end
        if newX ~= nil and newZ ~= nil then SetCamRot(Camera, vector3(newX, rotation.y, newZ), 2) end
    ...
    RunNoClipThread = function()
        Citizen.CreateThread(function()
            while IsNoClipping do
                ...
                if PlayerIsInVehicle == 1 then SetEntityAlpha(PlayerPed, NoClipAlpha, false) end
    ...
    StopNoClip = function()
        ...
        if GetVehiclePedIsIn(PlayerPed, false) ~= 0 then
            while (not IsVehicleOnAllWheels(NoClipEntity)) and not IsNoClipping do Wait(0) end
            while not IsNoClipping do
                Wait(0)
                if IsVehicleOnAllWheels(NoClipEntity) then return SetEntityInvincible(NoClipEntity, false) end
            end
        else
            if (IsPedFalling(NoClipEntity) and math.abs(1 - GetEntityHeightAboveGround(NoClipEntity)) > 1.00) then while (IsPedStopped(NoClipEntity) or not IsPedFalling(NoClipEntity)) and not IsNoClipping do Wait(0) end end
            while not IsNoClipping do
                Wait(0)
                if (not IsPedFalling(NoClipEntity)) and (not IsPedRagdoll(NoClipEntity)) then return SetEntityInvincible(NoClipEntity, false) end
            end
        end
    end
    ...
    ToggleNoClip = function(state)
        ...
        if IsNoClipping then RunNoClipThread() end
    end

    RegisterNetEvent('qb-admin:client:ToggleNoClip', function() ToggleNoClip(not IsNoClipping) end)
